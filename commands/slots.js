...
export async function execute(interaction){
  try{
    ...
    if(amount<=0||amount>MAX_BET) return interaction.editReply(`Bet must be between 1 and ${MAX_BET}.`);
    ...
    else if(row[0].emoji===row[1].emoji && row[0].multiplier2){ winnings=amount*row[0].multiplier2; winType="2 of a kind!"; }
    else if(row[1].emoji===row[2].emoji && row[1].multiplier2){ winnings=amount*row[1].multiplier2; winType="2 of a kind!"; }
    else if(row[0].emoji===row[2].emoji && row[0].multiplier2){ winnings=amount*row[0].multiplier2; winType="2 of a kind!"; }
    ...
    console.error(err);
    try{
      if(interaction.deferred) await interaction.editReply("Error running slots.");
      else await interaction.reply({content:"Error running slots.", ephemeral:true});
    } catch {}
  }
}
