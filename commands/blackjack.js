...
export async function execute(interaction) {
  ...
  try {
    ...
    const balance = await db.runTransaction(async t => {
      ...
      const current = doc.exists ? doc.data().balance : 1000;
    ...
    collector.on("end", async (_, reason) => {
      try {
        ...
        if (reason === "playerBust") {
          await finalize(false, pVal, dVal);
        } else {
          while (dVal < 17) {
            dealerHand.push(deck.pop());
            dVal = handValue(dealerHand);
          }
          const playerWin = (dVal > 21) || (pVal > dVal);
          await finalize(playerWin, pVal, dVal);
        }
      } catch (err) {
        console.error("End collector error:", err);
      }
    });

    async function finalize(playerWin, pVal, dVal) {
      ...
      const resultEmbed = new EmbedBuilder()
        ...
        .setFooter({ text: `Bet: $${amount} | Balance: $${currentBalance}` });

      if (!interaction.replied && !interaction.deferred) await interaction.reply({ embeds: [resultEmbed] });
      else await interaction.editReply({ embeds: [resultEmbed], components: [] });
    ...
    else await interaction.editReply({ content: "Error executing command.", components: [] });
  }
}
